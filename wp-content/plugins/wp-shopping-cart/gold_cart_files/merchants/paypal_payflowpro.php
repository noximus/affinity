...
function gateway_paypal_payflow($seperator, $sessionid) {
	...
	if($_POST['collected_data'][get_option('paypal_form_state')] != '') {
		$data['state'] = $wpdb->get_var("SELECT code FROM ".$wpdb->prefix."region_tax WHERE id='".$_SESSION['selected_region']."'");
	}
	...
	$email = $data['email'];
	$amount= nzshpcrt_overall_total_price($_SESSION['selected_country'],false,true);
	$amount= nzshpcrt_apply_coupon($amount,$_SESSION['coupon_num']);
	...
	$paypal_query_array = array(
		...
		'CLIENTIP'     => $cust_ip,
		'COMMENT1'   => $custom,
		'COMMENT2'   => 'usolympichalloffame.com',
		'INVNUM'     => $order_num,
		'ORDERDESC'  => $desc,
		'VERBOSITY'  => 'MEDIUM',
...
// 	echo ($result_code);
	if ($result_code == 1 || $result_code == 26) {
		echo "Account configuration issue.  Please verify your login credentials.";
...
// 				wpsc_member_activate_subscriptions($log_id);
				$wpdb->query("UPDATE `".$wpdb->prefix."purchase_logs` SET `paypal_recurring_profileid` = '".$nvpArray['PROFILEID']."' WHERE `id` = ".$log_id." LIMIT 1");
...
// 			wpsc_member_activate_subscriptions($log_id);
	...
	if ($result_code!=0) {
		...
		header("Location:".get_option('transact_url').$seperator."payflow=1&result=".$result_code."&message=1");
	} else {
		//header("Location:".get_option('transact_url').$seperator."payflow=1&message=1");
	}
